...
| P0 | Cross-scenario | `includeLocked=true`로 잠긴 증거 제목이 노출되는 문제가 유지됨 | fresh session 초반 조회에서 locked evidence의 title과 unlockHint가 반환됨 | 잠긴 증거는 placeholder 또는 개수만 노출 | description은 null이어도 title만으로 후반 추리 방향이 노출됨 | 증거 해금 전부터 물병/처방/마킹/잠금부 등 핵심 방향을 알 수 있음 | 일반 사용자 권한에서 `includeLocked=true` 차단 또는 title까지 마스킹 |
...
| P1 | Cross-scenario | 10일에 지적된 AI 회피 답변 문제가 11일에도 재현되고 결과 실패로 이어짐 | `단정할 수 없다`, `추가 증거 필요`, `기록을 함께 봐야 한다` 반복. 스튜디오9는 방법은 맞혔지만 attribution 실패 | 증거 제시 시 인정 가능한 사실과 다음 비교 대상을 제공 | 답변이 안전하지만 후보 귀속을 충분히 돕지 못함 | 사용자가 잘못된 후보로 확신할 수 있음 | prompt responseShape와 policy allowedFacts에 `인정 사실/부인 범위/다음 비교 대상` 강제 |
...
| P2 | Cross-scenario | Android chip prefill-only와 draft confirm은 미확인 | 터미널 API-only 환경 | chip tap, 자동 전송 없음, draft overwrite confirm 확인 | UI를 조작할 수 없어 확인 불가 | QA 핵심 항목 일부가 미검증 | Android 기기/에뮬레이터로 별도 retest 필요 |
...
```text
turns used: 50
candidate narrowing: 넓음 -> 물병/약통/2층 동선/기록 은폐 축 -> 2~3명 수준
...
```text
turns used: 50
candidate narrowing: 넓음 -> 조명/프롬프터/안전/마킹 조작 축 -> 3명 이상 상위 후보
...
| Android | P2 | chip prefill-only, draft overwrite confirm을 실제 기기에서 재검증 |
...
서월채:
  ...
  public-safe 판단: 물병/바이탈/기록 덮어쓰기 흐름이 가장 일관되어 제출 기준 충족

스튜디오9:
  추가 진행: 전체 증거 35/35 해금 후 80턴까지 심문
  최종 제출: 완료
  result API: 사용자 확인 요청 후 제출 이후에만 호출
  public-safe 판단: MARK 9 위치 이동과 현장 흔적 흐름이 가장 일관되어 제출 기준 충족
```
...
```text
서월채:
  result check: correct culprit, high score
  public-safe detail: method/evidence selection matched, motive/cover-up wording had minor misses

스튜디오9:
  result check: incorrect culprit, low score
  public-safe detail: method layer matched, culprit/motive/cover-up attribution missed
```

정답 인물명, 정답 해설 원문, raw result 전문은 public report에 포함하지 않는다.
...
| AI 답변 회피 반복 | 11일에도 50턴 내 확정 불가. extended에서도 스튜디오9 오답 | P1에서 더 강하게 봐야 함. 단순 UX 불편이 아니라 오답 attribution으로 이어짐 |
...
```text
서월채:
  10일에는 25턴/13개 증거 기준 오답 제출.
  11일에는 50턴 후 보류, 전체 증거 해금 후 extended 제출에서 정답 지목.
  개선은 guidance 때문이라기보다 전체 증거 해금과 추가 심문 덕분으로 보는 것이 타당하다.

스튜디오9:
  10일에는 Android E2E에서 정답 지목 성공.
  11일 API-only에서는 50턴 보류 후 extended 제출에서도 범인 지목 실패.
  따라서 스튜디오9는 "플레이 가능"과 별개로, guidance/prompt가 잘못된 레드헤링 귀속을 막아주지 못한다.
...
3. post-submit outcome contrast:
   - 서월채는 extended에서 correct/high score.
   - 스튜디오9는 method layer는 맞았지만 culprit attribution 실패.
   - 이 차이는 guidance/prompt가 "방법 계층"과 "범인 귀속"을 분리해 안내하지 못한다는 근거다.
...
AI 회피 답변:
  "개선 필요"가 아니라 "오답 귀속을 유발할 수 있는 P1"로 본다.
