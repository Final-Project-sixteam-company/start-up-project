...
class AiInterrogationServiceTest {
    ...
    @Test
    void interrogate_continuesRealAiCallWhenPromptContextLoggingFails() {
        InterrogationContextLoader contextLoader = mock(InterrogationContextLoader.class);
        AiPromptBuilder promptBuilder = mock(AiPromptBuilder.class);
        AiClient aiClient = mock(AiClient.class);
        MockResponseProvider mockResponseProvider = mock(MockResponseProvider.class);
        ...
        verify(aiClient).chatWithMetadata(
                eq("system prompt"), eq("user prompt"), any(AiRequestParams.class), any(AiCallContext.class));
        verify(logWriter).save(
                eq(sessionId),
                eq(suspectId),
                isNull(),
                eq(QuestionType.FREE),
                eq(request.question()),
                eq("답변"),
                eq("deepseek-v4-flash")
        );
